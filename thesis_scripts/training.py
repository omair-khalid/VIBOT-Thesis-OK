#!/usr/bin/env python
# -*- coding: utf-8 -*-
# Original Authors: Olexa Bilaniuk, Chiheb Trabelsi, Sandeep Subramanian
# Adpated by: Omair Khalid, for Master Thesis 
...
import h5py                                  as     H
import keras
from   keras.callbacks                       import Callback, ModelCheckpoint, LearningRateScheduler, LambdaCallback
from   keras.datasets                        import cifar10, cifar100
from   keras.initializers                    import Orthogonal
from   keras.layers                          import Layer, AveragePooling2D, AveragePooling3D, add, Add, concatenate, Concatenate, Input, Flatten, Dense, Convolution2D, BatchNormalization, Lambda, Activation, Reshape, ConvLSTM2D, Conv2D
...
import theano                                as     T
from data_prep import data_radar
from modReLU import modReLU
from zReLU import zReLU
#%%
...
def getResidualBlock(I, filter_size, featmaps, stage, block, shortcut, convArgs, bnArgs, d):
	...
	if   d.model == "real":
		...
		#O = Activation(activation)(O)
		O = zReLU()(O)#Activation(activation)(O)
...
def getResnetModel(d):
	...
	advanced_act  = d.aact
	drop_prob     = d.dropout
	inputShape    = (2, 32, 32) if K.image_dim_ordering() == "th" else (32, 32, 2)
	#if dataset == 'radar-cartesian':
     	#inputShape    = (2, 28, 28) if K.image_dim_ordering() == "th" else (28, 28, 2)
	#else:
		#sinputShape    = (2, 331, 180) if K.image_dim_ordering() == "th" else (331, 180, 2)
	...
	if d.model == 'real':
		O = Conv2D(sf, filsize, name='conv1_inrealmodel', **convArgs)(I)
		O = BatchNormalization(name="bn_conv1_2a", **bnArgs)(O)
		O = Activation(activation)(O)
	else:
		O = ComplexConv2D(sf, filsize, name='conv1_incomplexmodel', **convArgs)(I)
...
#	Stage 4
	...
	elif dataset == 'dataset-1' or dataset == 'dataset-2' or dataset == 'dataset-3'or dataset == 'dataset-4':
         O = Dense(4, activation='softmax', kernel_regularizer=l2(0.0001))(O)	
	elif dataset == 'svhn' or dataset == 'mnist-phase':
         O = Dense(10,  activation='softmax', kernel_regularizer=l2(0.0001))(O)
	elif dataset == 'radar-cartesian-all' or dataset == 'radar-polar-all' or 'radar-cartesian-three'or 'radar-polar-three'or 'radar-cartesian-six'or 'radar-polar-six' or 'radar-cartesian-threetrainsixtest':
         print(O.shape)
         O = Dense(6,  activation='softmax', kernel_regularizer=l2(0.0001))(O)
...
class TestErrorCallback(Callback):
	...
	def on_epoch_end(self, epoch, logs={}):
    		...
    		L.getLogger("train").info("Epoch {:5d} train_loss: {}, train_acc: {}, val_loss: {}, val_acc: {}, test_loss: {}, test_acc: {}".format(
    		                          ...
    		                          test_loss,        test_acc))
    		L.getLogger("train").info("Epoch {:5d} train_loss: {}, train_acc: {}, test_loss: {}, test_acc: {}".format(epoch+1,
    		                          logs["loss"],     logs["acc"],
    		                          test_loss,        test_acc))
...
def train(d):
	...
	elif d.dataset == 'mnist-phase':
         X_train = np.load('/home/omair/MNIST_SYNTHETIC_DATASET_EXPERIMENT/MNISTphase_dataset/X_TRAIN.npy')
         y_train = np.load('/home/omair/MNIST_SYNTHETIC_DATASET_EXPERIMENT/MNISTphase_dataset/Y_TRAIN.npy')
         X_test = np.load('/home/omair/MNIST_SYNTHETIC_DATASET_EXPERIMENT/MNISTphase_dataset/X_TEST.npy')
         y_test = np.load('/home/omair/MNIST_SYNTHETIC_DATASET_EXPERIMENT/MNISTphase_dataset/Y_TEST.npy')
         #y_train                              = y_train - 1
         #y_test                               = y_test  - 1
         nb_classes                           = 10
         n_train                              = 45000
	...
	elif d.dataset == 'dataset-1':
         X_train = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_1/X_TRAIN.npy')
         y_train = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_1/Y_TRAIN.npy')
         X_test = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_1/X_TEST.npy')
         y_test = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_1/Y_TEST.npy')
         y_train                              = y_train - 1
         y_test                               = y_test  - 1
         nb_classes                           = 4
         n_train                              = 1200

	elif d.dataset == 'dataset-2':
         X_train = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_2/X_TRAIN.npy')
         y_train = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_2/Y_TRAIN.npy')
         X_test = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_2/X_TEST.npy')
         y_test = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_2/Y_TEST.npy')
         y_train                              = y_train - 1
         y_test                               = y_test  - 1
         nb_classes                           = 4
         n_train                              = 1200
	elif d.dataset == 'dataset-3':
         X_train = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_3/X_TRAIN.npy')
         y_train = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_3/Y_TRAIN.npy')
         X_test = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_3/X_TEST.npy')
         y_test = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_3/Y_TEST.npy')
         y_train                              = y_train - 1
         y_test                               = y_test  - 1
         nb_classes                           = 4
         n_train                              = 1200
	elif d.dataset == 'dataset-4':
         X_train = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_4/X_TRAIN.npy')
         y_train = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_4/Y_TRAIN.npy')
         X_test = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_4/X_TEST.npy')
         y_test = np.load('/home/omair/SYNTHETIC_DATASET_EXPERIMENT/DATASET_4/Y_TEST.npy')
         y_train                              = y_train - 1
         y_test                               = y_test  - 1
         nb_classes                           = 4
         n_train                              = 5500
	...
	if d.dataset == 'radar-cartesian' or d.dataset == 'radar-polar' or d.dataset == 'dataset-1'or d.dataset == 'dataset-2' or d.dataset == 'dataset-3'or d.dataset == 'dataset-4' or d.dataset == 'mnist-phase':
	...
	model_details = model.fit_generator(generator       = datagen.flow(X_train, Y_train, batch_size=d.batch_size),
	...
	train_loss=model_details.history['loss']
	val_loss=model_details.history['val_loss']
	train_acc=model_details.history['acc']
	val_acc=model_details.history['val_acc']
	xc=range(d.num_epochs)
	...
	# - Baseline but with spectral pooling
 	...
 	#Plot tran vs val acc, train vs val loss, and the confusion matrix 
